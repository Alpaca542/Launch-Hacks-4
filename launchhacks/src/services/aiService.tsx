...
const ICON_MODEL = "gpt-5-nano";
const LAYOUT_MODEL = "gpt-5-mini";
const SUGGESTIONS_MODEL = "gpt-5-nano";
            ...
            while ((idx = buffer.indexOf("\n\n")) !== -1) {
                ...
                for (const line of rawEvent.split("\n")) {
                    ...
                    try {
                        ...
                        // Some runtimes send the completed function call as an output item with the
                        // full arguments string embedded at evt.item.arguments (e.g. response.output_item.done).
                        // Handle that shape explicitly: parse evt.item.arguments and invoke the tool hook.
                        if (
                            ...
                            if (itemId && handledItems.has(itemId)) {
                                continue;
                            }
                            ...
                            try {
                                parsedArgs =
                                    typeof rawArgsStr === "string"
                                        ? JSON.parse(rawArgsStr)
                                        : rawArgsStr;
                            } catch (parseErr) {
                                // Keep raw string if parsing fails
                                parsedArgs = rawArgsStr;
                            }
                            ...
                            if (itemId) handledItems.add(itemId);
                            ...
                            // ensure any arg buffer is cleared
                            if (itemId) argBuffers.delete(itemId);
                            continue;
                        }
